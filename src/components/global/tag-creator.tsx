...
const TagCreator = ({ defaultTags, getSelectedTags, subAccountId }: Props) => {
  ...
  const handleAddTag = async () => {
    try {
      ...
      setValue('');
      setSelectedColor('');
      const repsonse = await upsertTag(subAccountId, tagData);
      ...
      await saveActivityLogsNotification({
        agencyId: undefined,
        description: `Tag created | ${repsonse.name}`,
        subAccountId: subAccountId,
      });
    } catch (err) {
      toast({
  ...
  const handleDeleteTag = async (tagId: string) => {
    try {
      ...
      toast({
        title: 'Deleted Tag',
        description: 'Tag is deleted from your sub account',
      });
      await saveActivityLogsNotification({
      ...
      toast({
        title: 'Oops',
        variant: 'destructive',
        description: "Couldn't delete tag",
      });
    }
  };
